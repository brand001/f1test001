...
import { EventInfo, EventChangeType, SpecialUpdateType, VendorMarkets } from "$Utils/SbSportsBridge";
                    ...
                    if (changeData.ChangeType === EventChangeType.Update) {
                        changeData.SpecialUpdates.map(sUpdateData => {
                            ...
                            if (sUpdateData.UpdateType === SpecialUpdateType.HomeGoal) {
                                ...
                                this.setState({
                                    notificationInfo: thisEvent.HomeTeamName + "(进球) " + (thisEvent.HomeScore ?? 0) + " - " + (thisEvent.AwayScore ?? 0) + " " + thisEvent.AwayTeamName,
                                });
                            ...
                            if (sUpdateData.UpdateType === SpecialUpdateType.AwayGoal) {
                                ...
                                this.setState({
                                    notificationInfo: thisEvent.HomeTeamName + " " + (thisEvent.HomeScore ?? 0) + " - " + (thisEvent.AwayScore ?? 0) + " " + thisEvent.AwayTeamName + "(进球)",
                                });
...
export class PushLayout extends React.Component {
    ...
    render() {
        return (
            <View>
                {this.props.showPush && window.notificationInfo != "" ? (
                ...
                {this.props.showPush && this.state.shows && window.notificationRecommend != "" ? (
                    <View style={styles.msgPush}>
                        <Touch
                            onPress={() => {
                                this.goMsgDetail();
                            }}
                            style={styles.msgPushTxt}>
                            ...
                            <View>
                                ...
                                <View
                                    ...
                                    <HTMLView value={`<div>${window.notificationRecommend.info}</div>`} style={{ width: width * 0.7 }} stylesheet={styleHtmls} />
                                </View>
                            </View>
                        </Touch>
                        <CloseIcon
