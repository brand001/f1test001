...
import { TransHistoryPage } from "central-payment-m23";
...
class Records_THVN extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            configSet: false,
    ...
    async componentDidMount() {
        const privateParams = {
            ...
            goRecord: ({ transactionId, reportType = "deposit" }) => {
                Actions.pop();
                Actions.Recordes({ reportType, transactionId });
            },
    ...
    render() {
        ...
        const { reportType = null, transactionId = null } = this.props;
        return (
            <>
                <TransHistoryPage reportType={reportType} transactionId={transactionId} />
            </>
        );
    }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(Records_THVN);