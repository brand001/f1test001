...
import { WrapView, GetBonusGmt, FormatDate } from "$Utils";
...
class News extends React.Component {
    ...
    renderHeader() {
        let { PersonalUnreadCount, TransferUnreadCount, AnnouncementUnreadCount } = this.state;
            ...
            title: () => {
                return (
                    <NavTab
                        ...
                        renderIcon={key => {
                            return key == 0 && (PersonalUnreadCount > 0 || TransferUnreadCount > 0 || AnnouncementUnreadCount > 0) &&
    ...
    GetTransferData = () => {
        ...
        let fetchurl =
            ...
            "?MessageTypeID=2&messageTypeOptionIdList=0&PageSize=8&PageIndex=1&";
        return window.fetchRequest(fetchurl, "GET").then((res) => {
            if (res && res.isSuccess && res.result) {
    ...
    GetPersonalData = () => {
        let fetchurl =
            ApiPortSB.GetMessages +
            "?MessageTypeID=1&messageTypeOptionIdList=0&PageSize=8&PageIndex=1&";
        return window.fetchRequest(fetchurl, "GET").then((res) => {
            if (res && res.isSuccess && res.result) {
    ...
    SetNews = (type, res) => {
        ...
        let pageTotal = (res.totalGrandRecordCount !== null && res.totalGrandRecordCount !== undefined)
            ? Math.ceil(res.totalGrandRecordCount / 8)
            : 1;

        this.setState({
            [`${type}Data`]: data,
            [`${type}UnreadCount`]: res.totalUnreadCount || 0,
            [`${type}PageInfo`]: {
                currentIndex: 1,
                lastPage: pageTotal,
            },
    ...
    getDetail = (type, data) => {
        ...
        Toasts.loading("加载中,请稍候...", 200);
        window.fetchRequest(fetchurl, "GET", "", 50000, type === "Announcement")
            .then((res) => {
                Toasts.removeAll();
                if (res && res.isSuccess && res.result) {
                    let detail = type == "Announcement" ? res.result.announcementResponse : res.result.inboxMessagesDetail;
    ...
    update = (type, fetchData, data, isSingle) => {
        ...
        Toasts.loading("请稍候...", 200);
        window.fetchRequest(ApiPortSB[`Update${fetchstring}`], "PATCH", fetchData)
            .then((res) => {
                Toasts.removeAll();
                if (res && res.isSuccess && res.result) {
                    if (isSingle) {
    ...
    getNextPage = (type) => {
        ...
        switch (type) {
            case "Personal":
                fetchurl = `${ApiPortSB.GetMessages
                    }?MessageTypeID=1&messageTypeOptionIdList=0&PageSize=8&PageIndex=${PersonalPageInfo.currentIndex * 1 + 1
                    }&`;
                break;
            case "Transfer":
                fetchurl = `${ApiPortSB.GetMessages
                    }?MessageTypeID=2&messageTypeOptionIdList=0&PageSize=8&PageIndex=${TransferPageInfo.currentIndex * 1 + 1
                    }&`;
                break;
            case "Announcement":
        ...
        this.setState(
            ...
            () => {
                Toasts.loading("加载中,请稍候...", 200);
                window.fetchRequest(fetchurl, "GET", "", 50000, type === "Announcement" ? true : false)
                    .then((res) => {
                        Toasts.removeAll();
                        if (res && res.isSuccess && res.result) {
                            let data = (type === "Announcement" ? res.announcementsByMember : res.inboxMessagesListItem) || [];
    ...
    MessageItem(item, type) {
        let imgs = ImagesUrl.newsMsgOther;
        if (type == "Personal") {
            imgs = ImagesUrl.newsMsgOther;
        } else if (type == "Transfer") {
            if (item.messageTypeOptionId == 3) {
                imgs = ImageMap.newsMsg3;
            } else if (item.messageTypeOptionId == 4) {
                imgs = ImagesUrl.newsMsg5;
            } else if (item.messageTypeOptionId == 5) {
                imgs = ImagesUrl.newsMsg5;
            } else {
                imgs = ImagesUrl.newsMsg6;
            }
        } else if (type == "Announcement") {
            if (item.newsTemplateCategory == 7) {
                imgs = ImagesUrl.newsAnnouncement7;
            } else if (item.newsTemplateCategory == 8) {
                imgs = ImagesUrl.newsAnnouncement8;
            } else if (item.newsTemplateCategory == 9) {
                imgs = ImagesUrl.newsAnnouncement9;
            } else {
                imgs = ImagesUrl.newsOther;
            }
        }
            ...
            <Touch onPress={() => { this.goDetail(type, item); }} style={[styles.messageItem, { backgroundColor: item.isRead ? "#e6e6ee" : "#fff" }]}>
                ...
                <View>
                    <View style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexDirection: "row" }}>
                        ...
                        <Text style={{ color: "#BCBEC3", fontSize: 11 }}>{FormatDate(moment.utc(item.sendOn).utcOffset(8))}</Text>
                    </View>
