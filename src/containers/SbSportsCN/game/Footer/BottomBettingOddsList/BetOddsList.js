...
class BetOddsList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        ...
        return <>
            {Selections.map((item, index) => {
                ...
                const PARLAYODDSMIN = (BetActiveType == 2 || BetActiveType == 3) && (item.SelectionStatus !== SelectionStatusType.OK) && (item.SelectionStatus === SelectionStatusType.PARLAYODDSMIN);
                const PARLAYPAYOUTMAX = (BetActiveType == 2 || BetActiveType == 3) && (item.SelectionStatus !== SelectionStatusType.OK) && (item.SelectionStatus === SelectionStatusType.PARLAYPAYOUTMAX);
                ...
                return item ? (
                    <View key={index}>
                        <View style={[styles.Betinputlist, { width: width, }]}>
                            <View style={[styles.input_area, { width: width - 20, }]}>
                                {
                                    PARLAYODDSMIN ? <DisableBetBlock item={item} message={"赔率小于串关最小限制赔率"} /> :
                                        PARLAYPAYOUTMAX ? <DisableBetBlock item={item} message={"此串关组合达到赔付上限"} /> :
                                ...
                                <View style={[styles.itemRight, { width: width - 35 - 20, }]}>
                                    <View style={[styles.iteamName, { width: width - 55, }]}>
                                        ...
                                        <View>
											...
											<img src="/svg/betting/round-up.svg" /> */}
                                                    ...
                                                    <Image resizeMode='stretch' source={ImagesUrl.roundUpCn} style={{ width: 12, height: 12 }} />
                                                </View>
                                            ) : resultd ? (
                                                <View style={styles.OddsTxt}>
                                                    {/* @<span
											...
											<img src="/svg/betting/round-down.svg" /> */}
                                                    ...
                                                    <Image resizeMode='stretch' source={ImagesUrl.roundDownCn} style={{ width: 12, height: 12 }} />
                                                </View>
                                            ) : (
                                                <View style={styles.OddsTxt}>
                                    ...
                                    <View style={[styles.iteamInput, { width: width - 55, }]}>
                                        ...
                                        {BetActiveType === 1 && (
                                            <Touch
                                                style={[styles.InputBox, { width: width * 0.3, borderColor: this.state.ShowKeyboard ? "#00A6FF" : "#fff" }]}
                                                onPress={() => {
                                                    if (CantPlay) return;
                                                    this.setState({
                                                        ShowKeyboard: true,
                    ...
                    <View className="errortxt" key={index}><Text>此盘口关闭，或刷新试试！</Text></View>
                );
            })}
        </>;
    }
