    ...
    getGuide = (e) => {
        ...
        if (nativeEvent && nativeEvent.layout && nativeEvent.layout.y !== undefined) {
            this.setState({
                ...
                GuideLeft: nativeEvent.layout.x
            });
        }
    };
    getGuideReturn() {
                ...
                {SportDatas.length > 0 &&
                    <View>
                        <View style={styles.nav}>
                            ...
                            >
                                {SportDatas.map(item => {
                                    if (item.SportId != "2022" && item.Count <= 0) {
                                    ...
                                    return <>
                                        {
                                            (item.SportId == "2022") ? false ? null : null : (
                                                <Touch
                                                    key={item.SportId}
                                                    style={[styles.SportTab, (sportIsSelected ? styles.SportTabSelected : {})]}
                                                    onPress={() => this.clickSport(item)}
                                                >
                                                    <Text style={[styles.SportTabText, styles.SportTabTextCount, (sportIsSelected ? styles.SportTabTextCountSelected : {}), { color: "#999" }]}>{item.Count}</Text>
                                                    <Image resizeMode='stretch' source={sportIcon} style={{ width: 20, height: 20, marginBottom: 2 }} />
                                                    <Text style={[styles.SportTabText, , (sportIsSelected ? styles.SportTabTextSelected : {}), { color: "#666" }]}>{item.SportName}</Text>
                                                </Touch>
                                            )
                                        }
                                    </>;
