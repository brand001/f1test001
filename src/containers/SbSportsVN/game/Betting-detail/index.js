...
import { ArrowIcon, ShareIcon } from "$Components/icons/index";
...
class BettingDetail extends React.Component {
    constructor() {
        ...
        this.state = {
            ...
            isOpenBettingCart: false,
            siteUrl: window.SBTDomain + "/vn/mobile/sbtwo",
        };
    ...
    getSiteUrl = () => {
        return timeout_fetch(
            fetch("https://www.zdhrb62.com/CMSFiles/F1APP/F1RST3DIUMShare.json?v=" + Math.random(), { method: "GET" })
                .then(response => response.json())
                .then(data => {
                    console.log(data);
                    if (data.domain) {
                        this.setState({ siteUrl: data.domain });
                    }
                }), 5000)
            .catch(error => {
                // 错误处理
    ...
    clickOdds = (selectionData) => {
        if (!global.localStorage.getItem("loginStatus") == 1) {
            Orientation.lockToPortrait();
            setTimeout(() => {
                Actions.Login({ types: "login", from: "Betting-detail" });
        ...
        if (!isComboBet) { //單投
            ...
            if (!overMaxSelectionCount) {
                ...
                if (indexOf === -1) {
                    ...
                    Toasts.success("Thêm Thành Công");//添加成功
                } else {
                    this.RemoveBetCart(selectionData);
                }
            } else {
                Toasts.fail("哎呀，最多可投注串关" + Vendor.configs.MaxParlay + "场呦");
            }
        }
    };
    ...
    _onLayout(event) {
        ...
        if (height > width) {
            ...
            console.log("this.props.headerWidth1", winHeight);
    ...
    clickLineGroup = (LineGroupId = null) => {
        ...
        if (LineGroupId !== null) {
            PiwikEventDataHandle("SbSportsVN_MatchpageOdds");
        }
    };
    ...
    getShareUrl = (Vendor, EventData) => {
        return this.state.siteUrl
            + `/sbtwo/?deeplink=${Vendor.configs.VendorName.toLowerCase()}&sid=${EventData.SportId}&eid=${EventData.EventId}&lid=${EventData.LeagueId}&aff=${affCodeKex}`;
    };
    ...
    showSharePopup = () => {
        ...
        PiwikEventDataHandle({
            category: "Navigation",
            action: "Click",
            name: "Share_" + thisEventId,
        });


        GetGlobalModal({
            modalData: {
                fromPage: "BettingDetailPage",
                title: EventDetail?.LeagueName,
                text: `${EventDetail?.HomeTeamName} vs ${EventDetail?.AwayTeamName}`,

                renderHeader: this.shareHeader({ EventDetail, Vendor }),
                copyUrl: this.getShareUrl(Vendor, EventDetail)
            },
            allowMask: true,
            name: "ShareModal",
            wrapStyle: { width: "100%" },
    ...
    showBettingCart(flag) {
        console.log("showBettingCart", flag);
        this.setState({ isOpenBettingCart: flag });
    }
    ...
    render() {
        ...
        return (
            <WrapView
                ...
                style={{ flex: 1, backgroundColor: "#00a6ff" }}
            >
                {/* {
                ...
                >
                        ...
                        )}
                    {
                        ...
                        <View style={isLandscape ? [styles.navRight, { right: DeviceInfoIos ? 8 : 0 }] : {}}>
                            <View style={[styles.topNav, { width: width, backgroundColor: isBlue ? "#00a6ff" : "#2C2C2E" }]}>
                                {
                                    !isLandscape &&
                                    <NavBack onPress={this.backToList} />
                                }
                                ...
                                <View style={[styles.headerTitle, isLandscape ? { width: width } : {}]}>
                                    {eventDatas && eventDatas.length > 1 ? (
                                        ...
                                        >
                                            ...
                                            <Text style={[styles.headerTitleTxt, { maxWidth: width * 0.5 }]} numberOfLines={1}>
                                                {eventDatas[0].LeagueName}
                                            </Text>
                                            <Image resizeMode='stretch' source={this.state.showDrawer ? ImagesUrl.ups : ImagesUrl.dow} style={{ width: 25, height: 25 }} />
                                        ...
                                        EventDetail != "" && EventDetail != null && (
                                            <View style={styles.headerTitle}>
                                                ...
                                                <Text style={[styles.headerTitleTxt, { maxWidth: width * 0.6 }]} numberOfLines={1}>{EventDetail.LeagueName}</Text>
                                            </View>
                                        )
                                    )}
                                </View>
                                { //猜冠軍不能分享
                                    (!isLandscape && EventDetail != "" && EventDetail != null && !EventDetail.IsOutRightEvent) ?
                                        <Touch onPress={this.showSharePopup}>
                                            <ShareIcon />
                                        </Touch> : <View style={{ width: 24, height: 24, opacity: 0 }}></View>
                                }
                            </View>
                        </View>
                    }
                    ...
                    >
                        <View style={[isLandscape ? [styles.headerDrawerRight, { right: DeviceInfoIos ? 8 : 0 }] : styles.headerDrawer]}>
                            ...
                            <View style={[styles.headerDrawerView, { width: width, height: 265 }]}>
                                <ScrollView
                                                ...
                                                <Touch onPress={() => {
                                                    ...
                                                    this.setState(
                                                        ...
                                                        () => {
                                                            ...
                                                            PiwikEventDataHandle("SbSportsVN_ViewMoreMatchPage");
                                                        }
                                                    );
                    ...
                    </Modal>
                    {
                        ...
                        // 			/> */}
                        // 			<Text style={{color: '#000'}}>{eventDatas[0].LeagueName}</Text>
                        // 			{/* <ReactSVG
                        ...
                        // 						thisClassName="Game-logo"
                        // 					/> */}
                        // 					<Text style={{color: '#000'}}>{EventDetail.LeagueName}</Text>
                        // 				</View>
                        // 			)
                        // 		)}
                        // </View>
                    ...
                    {EventDetail != "" && EventDetail != null ? EventDetail.IsOutRightEvent ? (
                                            ...
                                            return (
                                                <View style={styles.OddsList} key={k}>
                                                    <View style={styles.oddTeam}>
                                                        ...
                                                        <Image resizeMode='stretch' source={ImagesUrl.btiLogoVn} style={{ width: 25, height: 25 }} />
                        ...
                        <ScrollView
                            ...
                            style={{ backgroundColor: isBlue ? "#EFEFF4" : "#3A3A3C", height: height }}
                        >
                            {
                                isLandscape &&
                            ...
                            </View>
                            {
                                ...
                                <View style={{ flexDirection: "row" }}>
                                    <Touch onPress={() => { this.setState({ tabactive: 1 }); }} style={{ width: width * 0.5, backgroundColor: tabactive == 1 ? (isBlue ? "#fff" : "#3A3A3C") : (isBlue ? "#EDF0F6" : "#2C2C2E") }}>
                                        <Text style={{ color: tabactive == 1 ? "#00A6FF" : "#666666", lineHeight: 40, textAlign: "center", fontSize: 14, fontWeight: "600" }}>Tỷ Lệ Cược</Text>
                                        {/* 赔率 */}
                                    </Touch>
                                    <Touch onPress={() => {
                                        if (!this.state.infoTabIsEnabled) { return; }
                                        this.setState({ tabactive: 2 });
                                    }} style={{ width: width * 0.5, backgroundColor: tabactive == 2 ? (isBlue ? "#fff" : "#3A3A3C") : (isBlue ? "#EDF0F6" : "#2C2C2E") }}>
                                        <Text style={{ color: !this.state.infoTabIsEnabled ? "#BCBEC3" : tabactive == 2 ? "#00A6FF" : "#666666", lineHeight: 40, textAlign: "center", fontSize: 14, fontWeight: "600" }}>Lịch Sử Trận Đấu</Text>
                                        {/* 情报 */}
                                    </Touch>
                                </View>
                            }
                                ...
                                /> : null}
                            {
                                ...
                                >
                                    ...
                                    <View style={{ paddingBottom: 120, backgroundColor: isBlue ? "#EFEFF4" : "#3A3A3C" }}>
                                        {
                        ...
                        <View style={{ display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: "#FFFFFF", height: height - 50 }}>
                            <Image resizeMode='stretch' source={ImagesUrl.nodataVn} style={{ width: 230, height: 128 }} />
                            <Text style={{ color: "#999999", fontSize: 16, lineHeight: 40 }}>Không có dữ liệu...</Text>
                        </View>
                    )}
                    ...
                    <Bottombetbox
                        ...
                        OpenBettingCart={() => this.showBettingCart(true)}
                        closeBetCartPopup={() => this.showBettingCart(false)}
                        isLandscapeShow={setIsLandscape}
                    // OpenBettingCart={()=>this.setState({isOpenBettingCart:true})}
                    // closeBetCartPopup={()=>this.setState({isOpenBettingCart:false})}
                    />
                    ...
                    <Bottomsheet
                        ...
                        headerName="Tất Cả"//全部盘口
                        hasDefaultButton={true}
                        defaultButtonText={"Tất Cả " + (Lines ? Lines.length : 0)}
                ...
                {/* 投注购物车浮窗 */}
                <View style={{ position: "absolute", bottom: 60, right: isLandscape && setIsLandscape ? height * 0.54 : 35 }}>
                    {!isOpenBettingCart &&
                        <BetCartPopup
                            Vendor={this.props.Vendor}
...
export class CollapseList extends React.Component {
    ...
    render() {
        ...
        return (
            <View style={{ backgroundColor: isBlue ? "#FFF" : "#3A3A3C", width: this.props.detailWidth, borderRadius: 8, flex: 1, marginTop: 4, paddingBottom: 8 }}>
                <View style={styles.titleView}>
                    <TouchableOpacity style={styles.titles} onPress={() => { this.shows(); }}>
                        <ArrowIcon direction={this.state.shows ? "top" : "bottom"} fill="#A0A0A0" width={16} height={16}></ArrowIcon>
                        <Text style={{ color: isBlue ? "#000000" : "#F5F5F5", fontWeight: "500", paddingLeft: 10, width: this.props.detailWidth - 80 }}>{this.props.LeagueName}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => { this.props.changeTop(); }} style={{ display: "flex", justifyContent: "center", alignItems: "center", width: 32, height: 30, borderRadius: 32, backgroundColor: this.props.isPined ? "#00a6ff" : "#f7f7f7" }}>
                        <Image resizeMode='stretch' source={this.props.isPined ? ImagesUrl.goupActive : ImagesUrl.goup} style={{ width: 20, height: 20 }} />
                    </TouchableOpacity>
                </View>
                <View>
                    {
...
export class DetailLineGroupBar extends React.Component {
    render() {
        ...
        return (
            <View style={[styles.Detail_menuView, { width: width, backgroundColor: isBlue ? "#fff" : "#3A3A3C" }]}>
                <View style={[styles.Detail_menu, { width: width, backgroundColor: isBlue ? "#fff" : "#3A3A3C" }]}>
                    <Touch
                        style={selectedLineGroup === null ? styles.list_active : {
                            borderRadius: 30,
                            paddingHorizontal: 9,
                            paddingVertical: 6,
                            backgroundColor: isBlue ? "#ecedef" : "#545457",
                            marginLeft: 12,
                        }}
                        onPress={() => onClickLineGroup(null)}
                    >
                        <Text style={{
                            color: selectedLineGroup === null ? "#fff" : (isBlue ? "#999" : "#CCCCCC"), lineHeight: 18, fontSize: 12, fontWeight: "600"
                        }}>Tất Cả {Lines ? Lines.length : 0}</Text>
                        {/* 全部 */}
                    </Touch>
                    ...
                    <TouchableOpacity
                        style={[styles.moreBtn, { backgroundColor: isBlue ? "#ecedef" : "#545457" }]}
                        onPress={onClickLineGroupMore}
                    >
                        <Text style={{ lineHeight: 30, color: isBlue ? "#999" : "#CCCCCC", fontSize: 12 }}>+ Thêm</Text>
                        {/* +更多 */}
                    </TouchableOpacity>
                </View>
            </View>
