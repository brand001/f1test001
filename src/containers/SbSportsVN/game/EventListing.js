            ...
            <View style={{ flex: 1 }}>
                {
                    ...
                    <View style={styles.moreLodin}>
                        ...
                        <Text style={{ color: "#999", fontSize: 12 }}>tải thêm...</Text>
                        {/* 加载更多... */}
                    </View>
                }
                {
                    isMores && !hasMore &&
                    <View>
                        <Text style={{ color: "#999", fontSize: 12, lineHeight: 50, textAlign: "center" }}>không còn nữa</Text>
                        {/* 没有更多了 */}
                    </View>
                }
            </View>
                    ...
                    </View>}
                {
                    ...
                    <View>
                        <Nav
                            ...
                            gotoWorldCup={this.props.gotoWorldCup}
    ...
    MiniGamesBanners() {
        ...
        fetch(`${Strapi_Domain + ApiPort.CMS_BannerFeature}?login=${login}&displaying_webp`, {
            ...
            if (Array.isArray(data) && data.length) {
                let MiniGamesBanners = Boolean(data.find(v => v?.action?.url?.toLocaleUpperCase()?.includes("euro".toLocaleUpperCase())));
            ...
            <View style={{ flex: 1 }} onLayout={(e) => this.getGuide(e)}>
                ...
                }
                {
                    firstloade ?
                        ...
                        <FlatList
                            onScroll={(e) => {
                                if (Array.isArray(EventDatas) && EventDatas.length >= 4) {

                                    let offsetY = e.nativeEvent.contentOffset.y; //滑动距离
                                    let offsetH = layoutHeaderHeight;

                                    if (offsetY > offsetH && !navTop) {
                            ...
                            onEndReachedThreshold={0.01} //执行上啦的时候1%执行
                ...
                {/* 投注购物车浮窗 */}
                <View style={{ position: "absolute", bottom: 75, right: 10 }}>
                    <BetCartPopup
                        Vendor={this.props.Vendor}
                        ShowBottomSheet={(t) => {
                            this.BettingRef.ShowBottomSheet(t);
                ...
                {
                    MiniGamesBanners && isMiniGamesActiveGame &&
                    null
                }
                {
                    // <ScrollView
                ...
                {/* {console.log(12121,EventDatas.length)}
			    {EventDatas && EventDatas.length>0 &&<View style={{ position: 'absolute', bottom: 30,alignSelf:'center'}}>
					<BottomtimeIcon />
				</View>} */}
            </View>
        );
