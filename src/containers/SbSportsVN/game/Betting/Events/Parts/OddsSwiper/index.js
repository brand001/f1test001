...
class OddsSwiper extends React.Component {
    ...
    render() {
        ...
        if (EventData && EventData.Lines && EventData.Lines.length > 0) {
            ...
            //處理玩法："大/小"(over/under) 的名稱
            //M3 各vendor使用不同配置，需要分開處理
            let overUnderName = i18n.BIGSMALL;
            if (Vendor.configs.VendorName === "IM") {
                overUnderName = i18n.BIG_SMALL; //IM會將BIG_OR_SMALL置換為BIG_SMALL
            } else if (Vendor.configs.VendorName === "BTI") {
                overUnderName = i18n.BTI.BTIBetTypeNames["3_0"]; //BTI按照配置進行置換，都一樣翻譯，寫死其中一個就可以
            } else if (Vendor.configs.VendorName === "SABA") {
                overUnderName = i18n.SABA.SABABetTypeNames["3"]; //SABA按照配置進行置換，都一樣翻譯，寫死其中一個就可以
            }
            ...
            Periodlist.map(periodId => {
                ...
                const RightLines = EventData.Lines.filter(l => l.PeriodId === periodId && l.BetTypeName === overUnderName);
            ...
            <View>
                {
                    pairedLines.length > 0 ?
                            ...
                            >
                                {
                                    ...
                                    pairedLines.splice(0, 1).map((item, index) => {
                                        ...
                                        return (
                                            <View key={index} style={{ display: "flex", justifyContent: "center", alignContent: "center", width: width * 0.5, flexDirection: "row" }} >
                                                <LineColumn
                                                    Vendor={Vendor}
                        ...
                        :
                        <View>
                            <View style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center", height: 40 }}>
                            ...
                            <View style={{ display: "flex", justifyContent: "space-around", alignItems: "center", flexDirection: "row", width: width * 0.5, flexWrap: "wrap" }}>
                                {[1, 2, 3, 4].map((item, index) => {
                                    return (
                                        <View key={index} style={{ display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: isBlue ? "#f7f7f7" : "#545457", borderRadius: 5, height: 40, marginBottom: 5, width: width * 0.23 }}>
