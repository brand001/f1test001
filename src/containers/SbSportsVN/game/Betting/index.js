...
class Betting extends React.PureComponent {
    ...
    clickOdds = (selectionData) => {
        ...
        PiwikEventDataHandle("SbSportsVN_OddsMainpageVertical");
    ...
    gameList = ({ item, index }) => {
        ...
        if (leagueData.LeagueId === "separatorFav") {
            return <View key={leagueData.LeagueKey} style={[styles.gameSeparator, { backgroundColor: isBlue ? "transparent" : "#2C2C2E" }]}>
                <Image resizeMode='stretch' source={ImagesUrl.separatorFav} style={{ width: 18, height: 18 }} />
                <Text style={{ color: "#00A6FF", paddingLeft: 8, }}>Trận Đấu Theo Dõi</Text>
                {/* 关注赛事 */}
            </View>;
        }
        if (leagueData.LeagueId === "separatorRun") {
            return <View key={leagueData.LeagueKey} style={[styles.gameSeparator, { backgroundColor: isBlue ? "transparent" : "#2C2C2E" }]}>
                <Image resizeMode='stretch' source={ImagesUrl.separatorRun} style={{ width: 24, height: 12 }} />
                <Text style={{ color: "#00A6FF", paddingLeft: 8, }}>Trực Tiếp</Text>
                {/* 滚球 */}
            </View>;
        }
        if (leagueData.LeagueId === "separatorToday") {
            return <View key={leagueData.LeagueKey} style={[styles.gameSeparator, { backgroundColor: isBlue ? "transparent" : "#2C2C2E" }]}>
                <CalendarIcon width={20} height={20} fill={"#0097E8"}></CalendarIcon>
                <Text style={{ color: "#00A6FF", paddingLeft: 8, }}>Sự Kiện Sắp Diễn Ra</Text>
                {/* 即将进行的赛事 */}
            </View>;
        }
        return (
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, backgroundColor: "rgba(233,233,238,1)" }}>
                {Loading ? (
                    <Skeleton EventsOnly={true} />
                ) : EventDatas == "" ? (
                    BetType.MarketId == 4 ?
                        <View style={{ display: "flex", justifyContent: "center", alignItems: "center", paddingTop: 35, backgroundColor: isBlue ? "#EFEFF4" : "#2C2C2E" }}>
                            <Image
                                onLoadStart={() => { this.props.isMore(); }}
                                resizeMode='stretch'
                                source={ImagesUrl.nodataVn}
                                style={{ width: width * .6, height: width * .6 * .55 }}
                            />
                            <Text style={{ color: isBlue ? "#666666" : "#999999", paddingTop: 10, paddingHorizontal: 30, textAlign: "center", fontSize: 16 }}>Hiện Bạn vẫn chưa theo dõi trận đấu nào, hãy theo dõi ngay</Text>
                            {/* 暂无关注赛事，马上关注赛事吧 */}
                        </View>
                        :
                        <View style={{ display: "flex", justifyContent: "center", alignItems: "center", paddingTop: 35, backgroundColor: isBlue ? "#EFEFF4" : "#2C2C2E" }}>
                            <Image
                                ...
                                style={{ width: width * .6, height: width * .6 * .55, }}
                            />
                            <Text style={{ color: isBlue ? "#666666" : "#999999", paddingTop: 10, paddingHorizontal: 30, textAlign: "center", fontSize: 16 }}>Hiện không có trận đấu nào, Vui lòng{"\n"} quay lại sau.</Text>
                            {/* 哎呀！暂无相关赛事，换个游戏试试吧！ */}
                        </View>
                ) : (
                    <View>
