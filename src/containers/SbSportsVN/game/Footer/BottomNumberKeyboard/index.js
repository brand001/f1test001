    ...
    render() {
        ...
        return (
            <View style={[styles.Bet_digit_keyboard_area, { width: width - 20, }]}>
                ...
                <View style={[styles.number_area, { width: width - 20, backgroundColor: isBlue ? "#efeff4" : "#3A3A3C", }]}>
                    {/* 键盘列表 */}
                    {NumberList.map((item, index) => {
                        return (
                            <TouchableOpacity
                                style={[styles.numItemBtn, { width: width * 0.18, height: (width * 0.18) * 0.57, marginLeft: index == 3 || index == 7 || index == 11 || index == 15 ? 15 : 0, backgroundColor: isBlue ? "#fff" : "#2E2E2E" }]}
                                // className={
                                // 	index == 3 ? (
                                // 		'item btn'
                            ...
                            >
                                {index == 14 ? (
                                    ...
                                    <Image resizeMode='stretch' source={ImagesUrl.dlt} style={{ width: 22, height: 22 }} />
                                ) : index == 3 ? (
                                    <Image resizeMode='stretch' source={ImagesUrl.max} style={{ width: width * 0.18, height: (width * 0.18) * 0.57 }} />
                                )
                                    : (
                                        <Text style={{ color: index == 7 || index == 11 || index == 15 ? "#FFA82B" : (isBlue ? "#999999" : "#CCCCCC"), fontSize: 13, fontWeight: "bold" }}>
                                            {item.name}
                                        </Text>
                                    )
                                }
