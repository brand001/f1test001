...
import styles from "./styleType.js";
...
import { FailIcon, SuccessIcon, WarningIcon, RefreshIcon } from "$Components/icons/index.js";
class BetCart extends React.Component {
    ...
    NotifyBettingInfo = (ID, note = null, retry = 0) => {
        ...
        PiwikEventDataHandle({
            category: `${piwikHead}_START`,
            action: "touch",
            name: " ",
        });
        const that = this;
        const startT = new Date().getTime();
        window.fetchRequest(ApiPortSB.NotifyBettingInfo, "POST", postData, 30000) //30秒超時
            .then((data) => {
                const diffSeconds = ((new Date().getTime() - startT) / 1000).toFixed(2);
                if (!data || !data.result) {
                    PiwikEventDataHandle({
                        category: `${piwikHead}_END_${diffSeconds}_EMPTY`,
                        action: "touch",
                        name: " ",
                    });
                } else {
                    ...
                    PiwikEventDataHandle({
                        category: `${piwikHead}_END_${diffSeconds}_${jsonData}`,
                        action: "touch",
                        name: " ",
                    });
                }
            ...
            .catch(err => {
                const diffSeconds = ((new Date().getTime() - startT) / 1000).toFixed(2);
                PiwikEventDataHandle({
                    category: `${piwikHead}_ERROR_${diffSeconds}_${err}`,
                    action: "touch",
                    name: " ",
                });
                if (retry < 10) { //最多重試10次
    ...
    placeBet = async (
        ...
        const PIWIKBetResult = (isSuccess, wagerId = null) => {
            ...
            if (isSuccess) {
                PiwikEventDataHandle({
                    category: `BetSpeed_${this.props.Vendor.configs.VendorName}_${BetActiveType}_${diffSeconds}_${"Success"}_${wagerId}`,
                    action: "touch",
                    name: " ",
                });
                ...
                PiwikEventDataHandle({
                    category: `BetSpeed_${this.props.Vendor.configs.VendorName}_${BetActiveType}_${diffSeconds}_${"failure"}`,
                    action: "touch",
                    name: " ",
                });
        ...
        const betlog = (betid, logJSON) => {
            ...
            PiwikEventDataHandle({
                category: "betlog_" + this.props.Vendor.configs.VendorName + "_" + BetActiveType,
                action: betid + "",
                name: logJSON,
            }); //betid需要做成string型態，不然app會報錯
        };
        ...
        return new Promise((inner_resolve) => {
            ...
            this.props.Vendor
                        ...
                        this.props.Vendor.queryPendingBetStatus(betResult.PendingQueryId).then((queryBetStatusData) => {
                            ...
                            if (queryBetStatusData.IsSuccess) {
                                ...
                                if (queryBetStatusData.FinalWagerStatus != 1) {
                                    if (!this.isDidUnmount) {
                                        updateBetResultStatus(key, 3, "Đơn Cược bị hủy (Pha bóng nguy hiểm)"); //3失敗 投注已被取消(危险球)
                                    }
                                } else {
                                    //超時，把投注選項 從 購物車移除
                ...
                .catch((error) => {
                    ...
                    //這裡處理下注可能返回的錯誤
                    let msg = "Đường truyền bị gián đoạn";
                            ...
                            this.props.Vendor.getBetInfo(BetActiveType != 1 ? 2 : 1, betInfo.Selections)
                                .then(pr => {
                                    ...
                                    if (displayText && OldOdds && NewOdds && newOddsIsSmaller) {
                                        ...
                                        Alert.alert("",
                                            "Tỷ lệ cược đã thay đổi thấp hơn, Bạn có chấp nhận?"
                                            // + displayText
                                            ...
                                            , [
                                                {
                                                    text: "Chấp Nhận", onPress: async () => {//接受
                                                ...
                                                },
                                                {
                                                    text: "Hủy", //不接受
                                                    onPress: () => {
                                                        updateBetResultStatus(key, 3, "Đơn Cược bị hủy"); //赔率降低取消 投注失敗
                                                        PIWIKBetResult(false);
                                        ...
                                        throw "Đường truyền bị gián đoạn";//投注不成功，请稍后重试
                                    }
                                })
                                .catch(e => {
                                    console.log("===查詢新賠率失敗??", e);
                                    //發生錯誤就當作投注失敗
                                    updateBetResultStatus(key, 6, "Đường truyền bị gián đoạn"); //3投注失敗
                                    PIWIKBetResult(false);
                    ...
                    console.log("isUnknownError ", isUnknownError);
                    if (isUnknownError) {
                        updateBetResultStatus(key, 6, "Đường truyền bị gián đoạn"); //6未知失敗
                    } else {
    ...
    ClickTabmenu = (type) => {
        if (type == 1) {
            PiwikEventDataHandle("SbSportsVN_SingleBetBetCart");
        } else if (type == 2) {
            PiwikEventDataHandle("SbSportsVN_ComboBetBetCart");
        } else {
            PiwikEventDataHandle("SbSportsVN_SystemComboBetCart");
        }
                    ...
                    <View>
                        ...
                        <View style={[styles.headerTop, { width: width }]}>
                            <View style={{ display: "flex", justifyContent: "center", alignItems: "center", flexDirection: "row" }}>
                                <Text style={{ color: "#fff", fontSize: 16 }}>Phiếu Cược</Text>
                                {/* 投注单 */}
                            ...
                            <View style={{ display: "flex", justifyContent: "center", alignItems: "center", flexDirection: "row" }} >
                                {this.props.userInfo.isGettingBalance ? (
                                    <Text style={{ color: "#fff", paddingRight: 10 }}>Số dư đang được làm mới</Text>
                                    // 余额刷新中
                                ) : (
                                    <View style={{ borderRadius: 5, padding: 6 }}>
                                        <Text style={{ color: "#fff", marginRight: 10, fontSize: 18 }}>{numberWithCommas(this.props.userInfo.balanceSB)} đ</Text>
                                    </View>
                                )}
                                {this.props.userInfo.isGettingBalance ? (
                                    <View />
                                        ...
                                        onPress={() => {
                                            this.props
                                                ...
                                                .then((result) => {
                                                    ...
                                                    window.KeyBoardToast("success", "Làm Mới Thành Công");
                        ...
                        {this.props.userInfo.balanceSB < 20 &&
                            showBalanceError && (
                                <View style={[styles.ShowbalanceTxt, { width: width }]}>
                                    ...
                                    >
                                        <Text style={{ fontSize: 16, lineHeight: 50 }}>Số dư của Bạn còn lại rất ít,<Text style={{ color: "#00a6ff", fontSize: 16, lineHeight: 50 }}>Nạp Tiền Ngay</Text></Text>
                                    </Touch>
                                </View>
                            )}
                        <View
                        ...
                        >
                            <View style={styles.BetTabs}>
                                {[
                                    { name: "Cược Đơn", type: 1 }, //单项投注
                                    { name: "Cược Xiên", type: 2 }, //混合过关
                                    { name: "Cược Xiên Hệ Thống", type: 3 }//系统混合过关
                                ].map((item, index) => {
                                    return (
                                        <Touch
                                            key={index}
                                            style={BetActiveType == item.type ? [styles.ActiveMenu, { width: width / 3, backgroundColor: isBlue ? "#fff" : "#2C2C2E", }] : [styles.noActiveMenu, { width: width / 3, backgroundColor: isBlue ? "#efeff4" : "#000000" }]}
                                            onPress={() => {
                                                if (loading) return;
                                                /* 如果混合过关 没有数据 则让用户去添加 */
                                        ...
                                        >
                                            <Text style={{
                                                fontSize: 13,
                                                color: BetActiveType == item.type ? "#1CA6FC" : "#666666",
                                                letterSpacing: index == 2 ? -0.5 : 0,
                                                fontWeight: BetActiveType == item.type ? "bold" : "400"
                                            }}>{item.name}</Text>
                                        </Touch>
                                    );
                                })}
                            ...
                            {/*				}}*/}
                            {/*			>*/}
                            {/*				<ArrowIcon direction={'bottom'} fill="#A0A0A0" width={16} height={16}></ArrowIcon>*/}
                            {/*				<Text style={{color: '#000'}}>查看更多</Text>*/}
                            {/*			</View>*/}
                            {/*		)}*/}
                            {/*		{ToUp && (*/}
                            {/*			<View*/}
                    ...
                    <View style={styles.Beting_Box_Status}>
                        <View style={[styles.Header, { width: width, }]}>
                            {(StartBettingloading == 1 || StartBettingloading == 4) && (<>
                                ...
                                {/* 投注在确认中, 可到未结算注单中查看状态 */}
                                <Text style={styles.headerTxt}>{StartBettingloading == 4 ? "Phiếu Cược đang được xác nhận, vui lòng kiểm tra tình trạng ở mục Chưa Thanh Toán" : "Vui lòng chờ trong khi đơn cược của bạn được xử lý"}</Text>
                                {/* 请等待几秒，您的投注在处理中 */}
                            </>)}

                            {/* 投注成功 */}
                            {StartBettingloading == 2 && (<>
                                <SuccessIcon width={30} height={30} fill='#fff' checkColor={"#00A6FF"}></SuccessIcon>
                                <Text style={styles.headerTxt}>Cược Thành Công</Text>

                            </>)}

                            {StartBettingloading == 6 && (<>
                                <WarningIcon width={30} height={30} fill={"#EB2121"}></WarningIcon>
                                <Text style={styles.headerTxt}>Đường truyền bị gián đoạn</Text>
                            </>)}

                            {StartBettingloading == 3 && (<>
                                <FailIcon width={30} height={30} fill={"#EB2121"}></FailIcon>
                                <Text style={styles.headerTxt}>Đặt Cược Thất Bại</Text>
                                {/* 投注失败 */}
                            </>)}
                        </View>
                        <ScrollView style={{ flex: 1, backgroundColor: isBlue ? "#fff" : "#2C2C2E" }}>
                            <Betstatus
                                detailWidth={detailWidth}
                                detailHeight={detailHeight}
                                BettingList={BettingList}
                                BetActiveType={BetActiveType}
                            />
                            <View style={[StartBettingloading == 1 ? styles.BottomBtnLoding : styles.BottomBtn, { backgroundColor: isBlue ? "#fff" : "#2C2C2E" }]}>
                                {(StartBettingloading != 3 && StartBettingloading != 6) && (<>
                                    <Touch
                                        ...
                                        onPress={() => {
                                            ...
                                            this.props.betCart_setIsComboBet(false, Vendor.configs.VendorName);
                                            this.props.CloseBottomSheet();
                                            PiwikEventDataHandle("SbSportsVN_ContinueBetCart");
                                        }}
                                    >
                                        <Text style={styles.cantEnterTxt}>Tiếp Tục Cược</Text>
                                        {/* 继续我的投注 */}
                                    </Touch>
                                    <Touch
                                        ...
                                        onPress={() => {
                                            ...
                                            if (this.props.EuroCupBet || this.props.EuroCupBetDetail) {
                                                ...
                                                PiwikEventDataHandle("SbSportsVN_BackMainpage");
                                                Actions.betRecordSBWrap();
                                            }
                                        }}
                                    >
                                        <Text style={styles.cantEnterWhitebtnTxt}>Xem Lịch Sử Cược</Text>
                                        {/* 查看投注单 */}
                                    </Touch>
                                </>)}
                                {/* <button
                                ...
                                {StartBettingloading == 6 && (
                                    ...
                                    >
                                        <Text style={styles.cantEnterTxt}>Xem Lịch Sử Cược</Text>
                                        {/* 去投注記錄 */}
                                    </Touch>
                                )}
                                ...
                                {StartBettingloading == 3 && (
                                    ...
                                    >
                                        <Text style={styles.cantEnterTxt}>Đã Hiểu</Text>
                                        {/* 知道了 */}
                                    </Touch>
                                )}
                            </View>
