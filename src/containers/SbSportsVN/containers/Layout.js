...
import EventInfo from "../lib/vendor/data/EventInfo";
...
class Layout extends React.Component {
    ...
    constructor() {
        ...
        this.notification_init_delay_handler = null; //延遲獲取的setTimeout handler
    ...
    componentDidMount() {
        ...
        //進球通知,延迟3秒获取用户配置,
        this.notification_init_delay_handler = setTimeout(() => {
            this.notification();
        }, 3000);
    ...
    componentWillUnmount() {
        if (this.notification_init_delay_handler) {
            clearTimeout(this.notification_init_delay_handler); //還未執行，要清理掉 for 未等到執行就切換走的情況
        }
    ...
    notification() {
                ...
                if (changeData.ChangeType === EventChangeType.Update) {
                    changeData.SpecialUpdates.map(sUpdateData => {
                        ...
                        if (sUpdateData.UpdateType === SpecialUpdateType.HomeGoal) {
                            ...
                            this.setState({ notificationInfo: thisEvent.HomeTeamName + "(Mục tiêu) " + (thisEvent.HomeScore ?? 0) + " - " + (thisEvent.AwayScore ?? 0) + " " + thisEvent.AwayTeamName });

                            window.notificationInfo = thisEvent.HomeTeamName + "(Mục tiêu) " + (thisEvent.HomeScore ?? 0) + " - " + (thisEvent.AwayScore ?? 0) + " " + thisEvent.AwayTeamName;
                            this.shows(true);
                        ...
                        if (sUpdateData.UpdateType === SpecialUpdateType.AwayGoal) {
                            ...
                            this.setState({ notificationInfo: thisEvent.HomeTeamName + " " + (thisEvent.HomeScore ?? 0) + " - " + (thisEvent.AwayScore ?? 0) + " " + thisEvent.AwayTeamName + "(Mục tiêu)" });

                            window.notificationInfo = thisEvent.HomeTeamName + " " + (thisEvent.HomeScore ?? 0) + " - " + (thisEvent.AwayScore ?? 0) + " " + thisEvent.AwayTeamName + "(Mục tiêu)";
                            this.shows(true);
                            //考慮連續進球，先關閉現有的timeout
                            if (this.notification_timeout_handler) {
                                clearTimeout(this.notification_timeout_handler);
        ...
        }, getEventInfosFunc, true); // <== 查詢參數 獲取 比賽查詢依據(EventInfo) 的函數
    }
            ...
            <View>
                {
                    this.props.showPush &&
                        ...
                        <View style={styles.gamePush}>
                            <Image resizeMode='stretch' source={ImagesUrl.ballNotification} style={{ width: 36, height: 36 }} />
                    ...
                    this.props.showPush && this.state.shows &&
                        ...
                        <View style={styles.msgPush}>
                            <Touch onPress={() => { this.goMsgDetail(); }} style={styles.msgPushTxt}>
                                <Image resizeMode='stretch' source={ImagesUrl.newsMsgOther} style={{ width: 36, height: 36 }} />
                                <View>
                                    {
