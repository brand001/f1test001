...
import { Flex, WingBlank, WhiteSpace, Accordion, Tabs, TabPane, Drawer } from "@ant-design/react-native";
...
class CashOutPopup extends React.Component {
    ...
    render() {
        ...
        let statusName = "Thanh Toán Thành Công";//兑现成功
        if (wagerData) {
            if (wagerData.CashOutStatus == CashOutStatusType.FAIL) { //失敗
                ...
                statusName = "Thanh Toán Thất Bại";//兑现失败
            } else if (wagerData.CashOutStatus == CashOutStatusType.DECLINE) { //拒絕價格異動
                statusBgColor = styles.cashoutPopupStatusRedBg;
                statusTextColor = styles.cashoutPopupStatusRedText;
                statusName = "Hủy thanh toán";//兑现取消
            }
        }
        ...
        >
            ...
            {wagerData && <>
                                    ...
                                    <View>
                                        ...
                                        {val.WagerItems[0].LineDesc &&
                                            <View style={{ paddingTop: 5 }}>
                                                <Text style={{ color: "#999999", width: width * 0.6, fontSize: 14 }}>{val.WagerItems[0].LineDesc}</Text>
                                            </View>
                                        }
                                    </View>
                                ...
                                <View style={{ width: width - 16 * 4, backgroundColor: "#fff", borderColor: "#EFEFF4", borderTopWidth: 1, paddingTop: 6, paddingBottom: 6 }}>
                                    {val.WagerItems.map(item => (
                                        <View key={item.EventId} style={{ paddingTop: 10, paddingBottom: 10 }}>
                                            ...
                                            <Text style={{ color: "#999999", fontSize: 12, paddingTop: 5 }}>{val.WagerItems[0].LeagueName}</Text>
                                            <Text style={{ color: "#999999", fontSize: 12, paddingTop: 5 }}>thời gian bắt đầu：{val.WagerItems[0].getEventDateMoment().format("MM/DD HH:mm")}</Text>
                                            {/* 开赛时间： */}
                                        </View>
                                    ))}
                                    <View style={{ borderColor: "#dde0e6", borderTopWidth: 1, marginTop: 6, paddingTop: 16, paddingBottom: 16 }}>
                                        <View style={{ justifyContent: "space-between", alignItems: "center", flexDirection: "row" }}>
                                            <Text style={{ color: "#999", fontSize: 12 }}>Tiền Cược: <Text style={{ color: "#000", fontWeight: "bold" }}>{val.BetAmount} VNĐ</Text></Text>
                                            {/* 投注额： */}
                                            ...
                                            <Text style={{ color: "#999", fontSize: 12, paddingTop: 3 }}>Tiền Bán Cược:<Text style={{ color: "#000", fontWeight: "bold" }}>{val.CashOutPrice} VNĐ</Text></Text>
                                            {/* 兑现金额： */}
                                        </View>
                                    </View>
                                </View>
                                ...
                                <View style={{ justifyContent: "center", alignItems: "center", backgroundColor: "#fff", paddingTop: 20, paddingBottom: 16, borderRadius: 8 }}>
                                    <View style={{ flexDirection: "row", width: width - 16 * 4, top: -5 }}>
                                        <View style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "flex-start" }}>
                                            <View>
                                                <Text style={{ fontSize: 16, color: "#000" }}>Cược Xiên</Text>
                                                {/* 混合投注 */}
                                            </View>
                                    ...
                                    <View style={[{ width: width - 16 * 4, backgroundColor: "#fff", borderColor: "#EFEFF4", borderTopWidth: 1, paddingTop: 6, paddingBottom: 6 }, { paddingLeft: 8 }]}>
                                        {val.WagerItems.map(item => (
                                            <View key={item.EventId} style={{ paddingLeft: 18, paddingTop: 10, paddingBottom: 10, borderColor: "#dde0e6", borderBottomWidth: 1 }}>
                                                ...
                                                <Text style={{ color: "#BCBEC3", paddingTop: 2, fontSize: 12 }}>thời gian bắt đầu：{item.getEventDateMoment().format("MM/DD HH:mm")}</Text>
                                                {/* 开赛时间： */}
                                            </View>
                                        ))}
                                        ...
                                        <View style={{ marginTop: 6, paddingTop: 16, paddingBottom: 16 }}>
                                            <View style={{ justifyContent: "space-between", alignItems: "center", flexDirection: "row" }}>
                                                ...
                                                <Text style={{ color: "#999", fontSize: 12, paddingTop: 3 }}>Tiền Bán Cược: <Text style={{ color: "#eb2121", fontSize: 14, fontWeight: "bold" }}>{val.CashOutPrice} đ</Text></Text>
                                                {/* 兑现金额： */}
                                            </View>
                                        </View>
                ...
                {wagerData.CashOutStatus == CashOutStatusType.DONE && <View style={{ flexDirection: "row", alignSelf: "center" }}>
                    <Touch
                        style={[styles.cashoutPopupButtonClose, { width: 150, marginRight: 10 }]}
                        onPress={closePopup}
                    >
                        <Text style={styles.cashoutPopupButtonTextClose}>
                            Đóng
                        </Text>
                    </Touch>
                    <Touch
                        style={[styles.cashoutPopupButton, { width: 150 }]}
                        onPress={closePopup}
                    >
                ...
                }
                {
                    ...
                    >
                        <Text style={styles.cashoutPopupButtonText}>
                            {/* 查看已兑现单 */}
                            {/* {
            wagerData.CashOutStatus == CashOutStatusType.DONE && "Xem Lịch sử cược"
          } */}
                            {
                                wagerData.CashOutStatus == CashOutStatusType.FAIL && "Đóng"
                            }
                            {/* 关闭 */}
                        </Text>
                    </Touch>
