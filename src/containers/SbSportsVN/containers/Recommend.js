...
class Recommend extends React.Component {
    constructor(props) {
        super();
        this.state = {
            hotEventsVisible: props.showRecommend,
            hotEvents: "",
        };
    }
    async componentDidMount() {
        const res = await StorageUtil.load("hotEventsVisible3");
        if (!res && ApiPort.UserLogin) {
            this.showHotEvents();
        }
    }
    ...
    showHotEvents(key) {
        ...
        fetch(HostConfig.Config.CacheApi + "/hotevents/" + hotEventsVendorName)
            ...
            .then(jsonData => {
                ...
                StorageUtil.save({
                    key: "hotEventsVisible3",
                    data: "hotEventsVisible3",
                });
                this.props.changeRecommed(hotEventsVisible);
                this.setState({ hotEvents: jsonData.data, hotEventsVisible: hotEventsVisible });
            })
            .catch(() => null)
            .finally(() => {
                Toasts.removeAll();
        ...
        return (
            <View>
                <Modal
                    animationType="none"
                    transparent={true}
                    visible={this.props.showRecommend}
                ...
                >
                    <View style={styles.depModal}>
                                ...
                                <View>
                                    <View style={{ paddingTop: width * 0.35 }}>
                                        {hotEvents.length > 0 && hotEvents.map((v, i) => (
                                            <Touch key={i} onPress={() => {
                                                this.props.changeRecommed(false);
                                                this.setState({ hotEventsVisible: false }, () => {
                                                    PiwikEventDataHandle("SbSportsVN_HotMatches");
                                                    let dataList = {
                                                        eid: v.EventId,
                                                        sid: v.SportId,
                                                        lid: v.LeagueId,
                                                ...
                                                <View style={styles.itemsList}>
                                                    <View style={styles.itemName}>
                                                        ...
                                                        <Text style={{ color: "#999", fontSize: 12, paddingBottom: 10 }}>{v.LeagueName}</Text>
                                                    </View>
                                                    <View style={styles.BettingList}>
                                                        <View style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                                            ...
                                                            <Text style={{ width: (width - 50) * 0.35, color: "#000", textAlign: "center", fontSize: 12, paddingTop: 10 }}>{v.HomeTeamName}</Text>
                                                        </View>
                                                        <View style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                                        ...
                                                        <View style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                                            ...
                                                            <Text style={{ width: (width - 50) * 0.35, color: "#000", textAlign: "center", fontSize: 12, paddingTop: 10 }}>{v.AwayTeamName}</Text>
                                                        </View>
                                                    </View>
                                                </View>
                                            </Touch>
                                    ...
                                    <Text style={{ color: "#fff", fontSize: 12, textAlign: "center" }}>Xem thêm cược khác tại đây</Text>
                                    {/* 点击卡片查看更多投注 */}
                                </View>
                            </View>
                        </View>
                        <Touch onPress={() => {
                            this.props.changeRecommed(false);
                            window.SBRecommendCheckDone = true;
