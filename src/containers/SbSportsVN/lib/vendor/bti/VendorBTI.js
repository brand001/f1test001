...
class VendorBTI extends VendorBase {
   ...
   */
    ...
    // 判斷是否為三項玩法
    ifThreeWayLine(lineData) {
        const betTypeIdStr = lineData.BetTypeId.toString();
        // 如果包含底線，則取底線後的數字
        const lastPartOfBetTypeId = betTypeIdStr.includes("_") ? betTypeIdStr.split("_").pop() : betTypeIdStr;
        return BTIThreeWayIds.includes(parseInt(lastPartOfBetTypeId));
    }

    // 整理三項系列數據
    sortThreeWay(selections) {
        if (!selections || selections.length <= 0) {
            return [];
        }

        // 按照group和metadata.type升序排序
        // group是第幾行，type是第幾個
        selections.sort((a, b) => {
            const aType = a.ExtraInfo?.Metadata?.type ?? 0; // 如果a.metadata.type为空，则使用默认值0
            const bType = b.ExtraInfo?.Metadata?.type ?? 0; // 如果b.metadata.type为空，则使用默认值0
            return a.SelectionGroup !== b.SelectionGroup ? a.SelectionGroup - b.SelectionGroup : aType - bType;
        });
